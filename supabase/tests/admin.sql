...
do $$ begin
  ...
  if (select count(*) from public.platform_admins) <> 0 then raise exception 'TEST FAIL: tenant user reads platform_admins'; end if;
  if (select count(*) from public.admin_audit_log) <> 0 then raise exception 'TEST FAIL: tenant user reads admin_audit_log'; end if;
end $$;
...
do $$ begin
  ...
  if (select count(*) from public.platform_admins) < 1 then raise exception 'TEST FAIL: admin cannot read platform_admins'; end if;
  perform count(*) from public.admin_audit_log;  -- readable (no error) though empty
