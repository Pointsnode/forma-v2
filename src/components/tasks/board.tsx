...
const RAIL: Record<Col, string> = { pending: "bg-muted", working: "bg-ink", waiting: "bg-wine", completed: "bg-teal" };
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        {master && weddingsForFilter.length ? weddingsForFilter.map((w) => (
          <button key={w.id} onClick={() => toggle(wedFilter, w.id, setWedFilter)}
            className={cx("flex items-center gap-1.5 rounded-[var(--radius)] px-2.5 py-1 text-[11.5px]", wedFilter.has(w.id) ? "bg-ink text-bone" : "bg-bone text-muted hover:text-ink")}>
        ...
        {(["flagged", "overdue", "today"] as const).map((u) => (
          <button key={u} onClick={() => toggle(urgFilter, u, setUrgFilter)}
            className={cx("rounded-[var(--radius)] px-2.5 py-1 text-[11.5px]", urgFilter.has(u) ? "bg-wine text-bone" : "bg-bone text-wine hover:opacity-80")}>
            {t(`filter_${u}`)}
          </button>
        ))}
        <button onClick={() => setSheet({ mode: "new" })} className="ml-auto rounded-[var(--radius)] bg-ink px-3.5 py-2 text-[13px] text-bone">+ {t("newTask")}</button>
      </div>
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <section key={col}
              ...
              className={cx("rounded-[var(--radius)] bg-bone p-3 transition-colors", overCol === col && "ring-2 ring-hairline")}>
              <header className="mb-2 px-1">
                <div className="flex items-baseline gap-2">
                  <h3 className="font-display text-[15px] text-ink">{t(`col_${col}`)}</h3>
                  <span className="rounded-[var(--radius)] bg-bone px-2 py-0.5 text-[11px] text-muted">{cards.length}</span>
                </div>
...
function TaskChip({ card, col, master, t, onOpen, onEdit, onComplete, onReopen, onFlag, onDragStart }: {
  ...
  return (
    <div draggable onDragStart={onDragStart}
      className="flex items-stretch gap-2 overflow-hidden rounded-[var(--radius)] bg-bone">
      ...
      <div className="min-w-0 flex-1">
        ...
        <button onClick={onOpen} className="block w-full px-1.5 pt-2 text-left" title={card.title}>
          ...
          <span className="mt-1.5 flex flex-wrap items-center gap-1.5">
            {master && card.weddingInitials ? <span className="rounded bg-bone px-1.5 py-[3px] text-[11px] font-medium uppercase tracking-[0.05em] text-muted">{card.weddingInitials}</span> : null}
            {card.eventLabel ? <span className="rounded bg-bone px-1.5 py-[3px] text-[11px] text-taupe">{card.eventLabel}</span> : null}
...
function ActionBtn({ onClick, icon, label, tone, active = false }: { onClick: () => void; icon: string; label: string; tone: "sage" | "wine" | "muted"; active?: boolean }) {
  const cls = tone === "sage" ? "text-teal hover:bg-bone" : tone === "wine" ? "text-wine hover:bg-bone" : "text-taupe hover:bg-bone hover:text-ink";
  return (
    <button onClick={onClick} className={cx("flex min-h-[40px] flex-1 items-center justify-center gap-1 rounded-[var(--radius)] text-[12px] font-medium sm:min-h-[34px]", cls, active && "bg-bone")}>
...
function AssigneeChip({ card }: { card: TaskCardVM }) {
  ...
  if (card.assigneeKind === "team") return <span className="flex h-[19px] items-center rounded-[var(--radius)] bg-ink px-1.5 text-[11px] font-medium text-bone" title={label}>{initialsOf(label)}</span>;
  if (card.assigneeKind === "couple") return <span className="flex h-[19px] items-center rounded-[var(--radius)] border border-wine px-1.5 text-[11px] font-medium text-wine" title={label}>{initialsOf(card.weddingName ?? label)}</span>;
  return <span className="flex h-[19px] items-center rounded bg-bone px-1.5 text-[11px] font-medium text-taupe" title={label}>{initialsOf(label)}</span>;
}
...
function DuePill({ card, t }: { card: TaskCardVM; t: ReturnType<typeof useTranslations> }) {
  ...
  if (card.due_date === today) return <span className="rounded-[var(--radius)] border border-wine px-1.5 py-[2px] text-[11px] text-wine">{t("dueToday")}</span>;
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
      ...
      <div className="relative flex w-full max-w-[460px] flex-col gap-3 rounded-[var(--radius)] bg-bone p-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onClose} className="rounded-[var(--radius)] px-3 py-2 text-[13px] text-muted hover:text-ink">{t("cancel")}</button>
