...
export function AddBoard({ weddingId }: { weddingId: string }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="text-[13px] text-text-meta hover:text-text-primary">+ {t("addBoard")}</button>;
  return (
...
export function AddItem({ boardId, weddingId }: { boardId: string; weddingId: string }) {
  ...
  const [pending, start] = useTransition();
  if (!open) return <button onClick={() => setOpen(true)} className="text-[12px] text-text-meta hover:text-text-primary">+ {t("addItem")}</button>;
  return (
