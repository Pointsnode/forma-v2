...
export function DocUpload({ weddingId }: { weddingId: string }) {
  ...
  return (
    <form action={(fd) => start(async () => { const r = await uploadDocument(weddingId, fd); setErr(r.error ? t("uploadError") : null); })} className="flex flex-wrap items-end gap-2">
      <label className="flex flex-col gap-1"><span className="text-[11px] text-muted">{t("docTitle")}</span><input name="title" required className="rounded-lg bg-bone px-2.5 py-1.5 text-[13px] shadow-card outline-none" /></label>
