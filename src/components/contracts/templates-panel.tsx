...
function TemplateForm({ initial, onClose }: { initial?: TemplateVM; onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-[var(--radius)] bg-surface-card p-4">
      <p className="font-display text-[15px] text-text-primary">{initial ? t("editTemplate") : t("newTemplate")}</p>
      <div className="grid grid-cols-[1.4fr_1fr] gap-3">
        <label className="flex flex-col gap-1"><span className="text-[12px] text-text-meta">{t("tName")}</span>
          <input value={name} onChange={(e) => setName(e.target.value)} maxLength={120} className={inputCls} /></label>
        <label className="flex flex-col gap-1"><span className="text-[12px] text-text-meta">{t("tKind")}</span>
      ...
      <label className="flex flex-col gap-1"><span className="text-[12px] text-text-meta">{t("tBody")}</span>
  ...
  return (
    <Row className="-mx-2 rounded-[var(--radius)] px-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => setEditing(true)} className="text-[12.5px] text-text-meta hover:text-text-primary">{t("edit")}</button>
        <button
          ...
          className="text-[12.5px] text-text-meta hover:text-[color:var(--color-text-danger)] disabled:cursor-not-allowed disabled:opacity-40"
        >
          {t("delete")}
        </button>
      </div>
  ...
  return (
    <div>
      <div className="mb-1 flex items-baseline justify-between">
        ...
        {!creating ? <button onClick={() => setCreating(true)} className="text-[12.5px] text-[color:var(--color-text-danger)] hover:underline hover:underline-offset-2">+ {t("newTemplate")}</button> : null}
      </div>
      <p className="mb-3 text-[12.5px] text-text-meta">{t("templatesHint")}</p>
      {creating ? <div className="mb-3"><TemplateForm onClose={() => setCreating(false)} /></div> : null}
      {templates.length === 0 && !creating ? (
        <p className="py-4 text-center font-accent text-[15px] text-text-meta">{t("templatesEmpty")}</p>
      ) : (
