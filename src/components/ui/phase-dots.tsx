...
const LIGHT = { done: "bg-sage", now: "bg-wine", ahead: "bg-hairline" } as const;
const DARK = { done: "bg-sage", now: "bg-wine", ahead: "bg-hairline-dark" } as const;
  ...
  return (
    <span aria-hidden className="inline-flex items-center gap-[3px] align-middle">
      {phaseDots(phase).map((s, i) => (
        <i key={i} className={cx("inline-block h-[7px] w-[7px] rounded-full", map[s])} />
      ))}
    </span>
  );
}
