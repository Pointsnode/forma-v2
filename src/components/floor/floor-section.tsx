      ...
      <Card>
        ...
        {props.role === "staff" ? (
          <button disabled={pending} onClick={() => start(async () => { await ensurePlan(props.eventId, props.weddingId, t("planName", { event: props.eventLabel })); router.refresh(); })}
            className="rounded-full bg-ink px-4 py-2 text-[13px] text-bone">{t("createPlan")}</button>
        ) : null}
      </Card>
    );
  }
  ...
  return (
    <Card>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <div className="ml-2 flex overflow-hidden rounded-full bg-bone text-[12px]">
      ...
      {view === "canvas" ? (
        ...
        <div>
          ...
          <div className="flex flex-col gap-3">
            {props.tables.map((tb) => (
              <div key={tb.id} className="rounded-xl bg-bone p-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tb.seats.length === 0 ? <span className="text-[12px] text-muted">{t("emptyTable")}</span> : tb.seats.map((s) => (
                    <span key={s.seatNo} className="rounded-full bg-paper px-2 py-0.5 text-[11.5px] text-ink"><b className="text-taupe">{seatLabel(s.seatNo)}</b> · {s.name}{s.diet.length ? ` · ${s.diet.join(", ")}` : ""}</span>
                  ))}
                </div>
              </div>
            ))}
            ...
            {props.attendees.filter((a) => !a.seated).length ? (
              <div className="rounded-xl border border-hairline p-3">
                ...
                <div className="flex flex-wrap gap-1.5">{props.attendees.filter((a) => !a.seated).map((a) => <span key={a.guestId} className="rounded-full bg-sand-soft px-2 py-0.5 text-[11.5px] text-taupe">{a.name}</span>)}</div>
              </div>
            ) : null}
          </div>
        </div>
