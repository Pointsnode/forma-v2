    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        {vm.priceStrip.length > 0 && (
          <div className="mt-5">
            <p className="mb-2 text-[11px] uppercase tracking-[0.16em] text-text-meta">{t("priceStripCaption")}</p>
            <div className="flex flex-wrap items-stretch gap-2">
              {vm.priceStrip.map((s, i) => (
                <div key={i} className="flex items-center gap-2">
                  {i > 0 ? <span aria-hidden className="text-text-meta">→</span> : null}
                  <div className={cx("rounded-[var(--radius)] px-3 py-2", s.label === "final" ? "bg-surface-chrome text-bone" : "bg-surface-card")}>
                    ...
                    {s.deltaFmt ? <div className={cx("text-[11px]", s.label === "final" ? "text-bone/70" : s.deltaUp ? "text-[color:var(--color-text-danger)]" : "text-teal")}>{s.deltaFmt}</div> : null}
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card>
        ...
        {vm.timeline.length <= 1 && vm.rail.latestQuoteId == null ? (
          <p className="py-4 text-center font-accent text-[15px] italic text-text-meta">{t("emptyLedger")}</p>
        ) : (
...
function TimelineRow({ it, t }: { it: TimelineVM; t: (k: string, v?: Record<string, string | number>) => string }) {
    ...
    return (
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[14px] text-text-primary"><span className="font-medium">{t("openedTitle")}</span>{it.presenter ? ` · ${it.presenter}` : ""} <span className="text-text-meta">· {it.dateFmt}</span></p>
          {it.estimateFmt ? <p className="text-[12.5px] text-text-meta">{t("estimate")}: {it.estimateFmt}</p> : null}
          {it.events.length ? <p className="text-[12.5px] text-text-meta">{it.events.join(" · ")}</p> : null}
  ...
  if (it.kind === "quote") {
    return (
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="flex flex-wrap items-center gap-2 text-[14px] text-text-primary">
            ...
            <span className="text-text-meta">· {it.dateFmt}</span>
          </p>
          {it.validUntil ? <p className="text-[12.5px] text-text-meta">{t("validUntilOn", { date: it.validUntil })}</p> : null}
          {it.note ? <p className="text-[12.5px] text-taupe">{it.note}</p> : null}
          {it.url ? <a href={it.url} target="_blank" rel="noreferrer" className="text-[12.5px] font-medium text-text-primary underline-offset-2 hover:underline">{t("openPdf")} ↓</a> : null}
        </div>
      </div>
    );
  }
  if (it.kind === "proposal") {
    return (
      <div className="flex gap-3">
        <Dot tone="ink" />
        <div className="min-w-0 flex-1">
          <p className="text-[14px] text-text-primary"><span className="font-medium">{it.title}</span> <Badge tone="sand">{t(`propStatus_${it.status}`)}</Badge> <span className="text-text-meta">· {it.dateFmt}</span></p>
          {it.respondedFmt ? <p className="text-[12px] text-text-meta">{t("answered", { date: it.respondedFmt })}</p> : null}
        </div>
      </div>
    );
  }
  return (
    <div className="flex gap-3">
      <span className={cx("mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-[var(--radius)] text-[10px] font-semibold", it.isCouple ? "bg-wine text-bone" : "bg-champagne text-text-primary")}>{it.authorName.slice(0, 1).toUpperCase()}</span>
      <div className="min-w-0 flex-1">
        <p className="text-[12.5px] text-text-meta">{it.authorName} · {it.dateFmt}</p>
        <p className="rounded-[var(--radius)] bg-surface-card px-3 py-2 text-[14px] text-text-primary">{it.body}</p>
      </div>
    </div>
  );
}
...
function Dot({ tone }: { tone: BadgeTone }) {
  ...
  const bg: Record<BadgeTone, string> = { sand: "bg-champagne", wine: "bg-wine", sage: "bg-teal", maroon: "bg-wine", ink: "bg-surface-chrome" };
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {s === "booked" && (
          <>
            ...
            <Link href={`/wedding/${vm.weddingId}/documents`} className="inline-flex items-center rounded-[var(--radius)] px-5 py-2.5 text-[14px] text-text-primary hover:text-taupe">{t("addDocument")}</Link>
          </>
        )}

        {(s === "declined" || s === "archived") && (
          <Link href={`/wedding/${vm.weddingId}/vendors?present=${r.vendorId}`} className="inline-flex items-center rounded-[var(--radius)] bg-surface-chrome px-5 py-2.5 text-[14px] font-medium text-bone hover:opacity-90">{t("rePresent")}</Link>
        )}
...
function RecordQuoteForm({ quoteId, weddingId, t, pending, onDone }: { quoteId: string; weddingId: string; t: any; pending: boolean; onDone: () => void; run: (fn: () => Promise<LedgerResult>, after?: () => void) => void }) {
  ...
  const F = "w-full rounded-[var(--radius)] bg-surface-card px-3.5 py-2.5 text-[14px] text-text-primary focus:outline-none focus:ring-1 focus:ring-champagne";
  ...
  return (
    <div className="mt-4 grid gap-3 rounded-[var(--radius)] bg-surface-card/60 p-4 sm:max-w-md">
      <input className={F} inputMode="decimal" placeholder={t("amountPlaceholder")} value={amount} onChange={(e) => setAmount(e.target.value)} />
      <label className="text-[12px] text-text-meta">{t("validUntilLabel")}<input className={cx(F, "mt-1")} type="date" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} /></label>
      <input ref={fileRef} type="file" accept="application/pdf" className="text-[12.5px] text-text-primary file:mr-3 file:rounded-[var(--radius)] file:border-0 file:bg-surface-chrome file:px-3 file:py-1.5 file:text-bone" />
      <p className="-mt-1.5 text-[11.5px] text-text-meta">{t("pdfHint")}</p>
      <textarea className={cx(F, "min-h-[64px] resize-y")} placeholder={t("notePlaceholder")} value={note} onChange={(e) => setNote(e.target.value)} />
      {ferr ? <p className="text-[13px] text-[color:var(--color-text-danger)]">{ferr}</p> : null}
    ...
    <Card>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          disabled={pending}
          className={cx("rounded-[var(--radius)] px-4 py-2 text-[13px] disabled:opacity-50", declining ? "font-medium text-[color:var(--color-text-danger)]" : "text-text-meta hover:text-[color:var(--color-text-danger)]")}
        >
      ...
      {asking && (
        <div className="mt-3 grid gap-2 sm:max-w-md">
          <textarea className="min-h-[72px] w-full resize-y rounded-[var(--radius)] bg-surface-card px-3.5 py-2.5 text-[14px] text-text-primary focus:outline-none focus:ring-1 focus:ring-champagne" placeholder={t("askPlaceholder")} value={msg} onChange={(e) => setMsg(e.target.value)} />
