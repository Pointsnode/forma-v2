...
type Msg = { role: "planner" | "concierge"; content: string; draft?: DraftCard | null };
type DraftCard = { kind: string; id: string; title: string };
type Convo = { threadId: string | null; messages: Msg[]; pending: number };

export type ConciergeBubbleProps = {
...
// scope key from the path: /wedding/<id>/... → that wedding; else the orchestrator
...
export function ConciergeBubble({ weddings, usage: usage0 }: ConciergeBubbleProps) {
  ...
  const convo = convos[scopeKey] ?? { threadId: null, messages: [], pending: 0 };
  const pendingTotal = Object.values(convos).reduce((n, c) => n + c.pending, 0);

  const setConvo = useCallback((updater: (c: Convo) => Convo) => {
    setConvos((prev) => ({ ...prev, [scopeKey]: updater(prev[scopeKey] ?? { threadId: null, messages: [], pending: 0 }) }));
  }, [scopeKey]);
  ...
  async function send() {
    ...
    try {
      const res = await fetch("/api/concierge", {
        ...
        body: JSON.stringify({ threadId: convo.threadId, scope: { weddingId }, message }),
      });
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[60] flex flex-col items-end gap-3 print:hidden">
            ...
            {convo.messages.length === 0 ? (
              ...
              <div key={i} className={m.role === "planner" ? "flex justify-end" : ""}>
                <div className={m.role === "planner"
                  ...
                  {m.draft ? (
                    <Link href={draftHref(m.draft.kind, m.draft.id, weddingId)} onClick={() => setConvo((c) => ({ ...c, pending: Math.max(0, c.pending - 1) }))}
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="relative flex items-center gap-2 rounded-full bg-ink px-4 py-3 text-bone shadow-lift transition-transform hover:scale-[1.03]"
        title={t("helper")}
      >
        <Glyph />
...
function attachDraft(c: Convo, d: DraftCard): Convo {
  ...
  return { ...c, messages: msgs, pending: c.pending + 1 };
}

function Glyph() {
  return (
    <span aria-hidden className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[rgba(247,244,238,0.14)] font-display text-[13px] leading-none text-bone">c</span>
  );
}
