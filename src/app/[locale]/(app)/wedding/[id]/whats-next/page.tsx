      ...
      <div className="flex flex-col gap-4">
        {groups.map((grp) => (
          <Card key={grp.phase} className={cx(grp.locked && "opacity-50")}>
            <h3 className="flex items-baseline gap-3 font-display text-[19px] text-ink">
              <Check ok={grp.total > 0 && grp.done === grp.total} />
              {grp.title}
              <span className="text-[11.5px] font-normal text-muted">{grp.locked ? t("locked") : t("goalProgress", { done: grp.done, total: grp.total })}</span>
            </h3>
            {grp.locked ? (
              <p className="mt-2 text-[13px] text-muted">{t("lockedHint")}</p>
            ) : (
              <div className="mt-2">
                {grp.goals.map((goal) => (
                  <div key={goal.key} className="flex items-start gap-3 py-3 not-last:[box-shadow:inset_0_-1px_0_var(--color-hairline)]">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[13.5px] font-medium text-ink">{goal.title}{goal.override === "manual_done" ? <span className="ml-1.5 text-[11px] text-taupe">· {t("markedDone")}</span> : null}</p>
                      {goal.detail ? <p className="mt-0.5 text-[12px] text-muted">{goal.detail}</p> : null}
                      {goal.subs.length ? (
                    ...
                    {goal.link ? <Link href={goal.link.href} className="shrink-0 text-[12px] text-wine hover:underline">{goal.link.label}</Link> : null}
