...
export default async function VenuesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      <StudioTitleBand title={t("venues")} accent={t("venuesHint")} action={<Link href="/vendors/new?kind=venue"><Button variant="primary">{t("addVenue")}</Button></Link>} className="mt-1" />
      {venues.length === 0
