  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          {msg ? <span className={cx("text-[13px]", msg.tone === "ok" ? "text-teal" : "text-[color:var(--color-text-danger)]")}>{msg.text}</span> : null}
      ...
      {!published && (
        <Card>
          ...
          <p className="mb-3 mt-0.5 text-[12.5px] text-text-meta">{t("checklistHint")}</p>
          <ul className="grid gap-1.5 sm:grid-cols-2">
            {checks.map((c) => (
              <li key={c.key} className="flex items-center gap-2 text-[13.5px]">
                <span className={cx("inline-flex h-4 w-4 items-center justify-center rounded-[var(--radius)] text-[10px]", c.ok ? "bg-teal text-bone" : "bg-surface-card text-taupe")}>
                  {c.ok ? "✓" : "·"}
                </span>
                <span className={c.ok ? "text-text-primary" : "text-text-meta"}>{t(`check_${c.key}`)}</span>
              </li>
            ))}
          </ul>
        </Card>
      ...
      <Card>
        ...
        <p className="mb-3 mt-0.5 text-[12.5px] text-text-meta">{t("addressHint")}</p>
        <div className="flex items-center gap-2">
          <span className="text-[13px] text-text-meta">/p/</span>
      ...
      <Card>
        ...
        <div className="mb-4 flex items-end gap-3">
          <div className="h-28 w-44 overflow-hidden rounded-[var(--radius)] bg-surface-card">
            {imgUrl(hero) ? (
        ...
        <div className="flex flex-wrap gap-2.5">
          {gallery.map((g, i) => (
            <div key={g} className="group relative h-24 w-24 overflow-hidden rounded-[var(--radius)] bg-surface-card">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={imgUrl(g)!} alt="" className="h-full w-full object-cover" />
              <button
                type="button"
                onClick={() => setGallery((gs) => gs.filter((_, j) => j !== i))}
                className="absolute right-1 top-1 rounded-[var(--radius)] bg-surface-chrome/70 px-1.5 text-[11px] text-bone opacity-0 transition-opacity group-hover:opacity-100"
                aria-label={t("remove")}
              >
                ×
              </button>
          ...
          <button
            ...
            className="flex h-24 w-24 items-center justify-center rounded-[var(--radius)] bg-surface-card text-[13px] text-taupe hover:bg-surface-card disabled:opacity-50"
          >
      ...
      <Card>
        ...
        <p className="mb-3 mt-0.5 text-[12.5px] text-text-meta">{t("servicesHint")}</p>
        <div className="space-y-2">
          {services.map((s, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              />
              <button type="button" onClick={() => setServices((ss) => ss.filter((_, j) => j !== i))} className="shrink-0 text-[16px] text-text-meta hover:text-[color:var(--color-text-danger)]" aria-label={t("remove")}>
                ×
              </button>
            </div>
          ))}
      ...
      <Card>
        ...
        <p className="mb-3 mt-0.5 text-[12.5px] text-text-meta">{t("areasHint")}</p>
        <div className="space-y-2">
          {areas.map((a, i) => (
            <div key={i} className="grid grid-cols-[1fr_1fr_1fr_auto] items-center gap-2">
              ...
              <button type="button" onClick={() => setAreas((as) => as.filter((_, j) => j !== i))} className="shrink-0 text-[16px] text-text-meta hover:text-[color:var(--color-text-danger)]" aria-label={t("remove")}>
                ×
              </button>
            </div>
          ))}
      ...
      <Card className="space-y-4">
        ...
        <div>
          ...
          <p className="mt-1 text-[11.5px] text-text-meta">{t("calendlyHint")}</p>
        </div>
    ...
    <div>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {(["en", "es"] as const).map((loc) => (
          <div key={loc}>
            <span className="mb-1 block text-[10.5px] uppercase tracking-[0.2em] text-text-meta">{loc}</span>
            <textarea
              rows={rows}
              className="w-full resize-y rounded-[var(--radius)] bg-surface-card px-3.5 py-2.5 text-[14px] text-text-primary focus:outline-none focus:ring-1 focus:ring-champagne"
              value={value[loc]}
