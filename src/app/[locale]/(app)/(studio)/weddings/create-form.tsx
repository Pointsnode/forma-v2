...
const inputCls =
  "w-full rounded-[var(--radius)] bg-surface-card px-3.5 py-2.5 text-[14px] text-text-primary outline-none";

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="flex flex-col gap-1.5">
      <span className="text-[13px] text-text-meta">{label}</span>
      {children}
    </label>
  );
}
  ...
  return (
    <form action={action} className="flex flex-col gap-4">
      ...
      <fieldset className="flex flex-col gap-1.5">
        <span className="text-[13px] text-text-meta">{t("kind")}</span>
        <div className="flex gap-2">
          {(["city", "destination"] as const).map((k, i) => (
            <label key={k} className="flex-1 cursor-pointer">
              ...
              <span className="block rounded-[var(--radius)] bg-surface-card px-4 py-3 text-center text-[14px] text-text-meta peer-checked:bg-surface-chrome peer-checked:text-bone">
